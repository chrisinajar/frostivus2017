...
test('can read in tooltip list', function (t) {
  ...
  t.ok(isFinite(Object.keys(getTranslations().lang.Tokens.values).length), 'there are tokens');
  t.end();
});
...
test('can parse item', function (t) {
  if (!itemPaths.length) {
    t.pass('There are no items to test. Add one');
    t.end();
    return;
  }
